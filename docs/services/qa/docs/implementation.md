...
| retrieval test / metrics | `services/qa/internal/http/resource_handlers.go` | Gateway OpenAPI | resource tests | Retrieval test 随 `qa:use` 对普通用户开放并按 owner 读取；metrics 仍是管理面；依赖 Knowledge retrieval client；`scoreThreshold: 0` 是有效显式配置，不应被 fallback 覆盖。 |
...
| Gateway -> Knowledge -> QA RAG smoke | `services/knowledge/internal/integration/gateway_rag_e2e_smoke_test.go`、`docs/runbooks/local-integration.md` | #304 / QA RAG 主链路 | `GATEWAY_RAG_E2E_SMOKE=1 go test ./internal/integration -run '^TestGatewayRAGE2ESmoke$' -count=1 -v` | 默认 skip；显式启用时通过 Gateway 配置 QA LLM/retrieval 版本，创建 QA session/message，要求模型使用 Knowledge 检索工具并返回包含 `RAG-E2E-304` 的 answer，同时校验 response citations 和 message citation list 匹配本轮 KB/doc/chunk。当前本地默认优先通过 Knowledge MCP `knowledge__search`，并保留内置 `search_knowledge` fallback。需要可用 AI Gateway chat profile/provider；不替代 QA 单服务 smoke 或 #125。 |
...
| Knowledge MCP 真实 Agent E2E 仍需扩大 | Knowledge MCP docs / #505 / #528 | QA / MCP / citation | Knowledge MCP server 已收敛为四个只读工具，QA 默认白名单包含 `knowledge__search`、`knowledge__list_documents`、`knowledge__get_document`、`knowledge__get_chunk`，并保留内置 `search_knowledge` fallback；完整 #125/前端/真实 provider smoke 仍需后续任务。 |
...
| Knowledge MCP 工具命名 | #528/#529 定义四个 `knowledge__*` 目标模型工具 | Knowledge MCP 远程 server 原生工具为 `search`、`list_documents`、`get_document`、`get_chunk`；QA alias 后暴露为 `knowledge__search` 等目标名称。QA active config 默认启用这些工具，并保留内置 `search_knowledge` fallback | 与目标命名一致；剩余风险在真实模型是否稳定选择工具及完整 #125 覆盖 | 保留直接 MCP smoke、Gateway retrieval-test 和后续真实 provider Agent E2E 证据。 |
...
| Gateway -> Knowledge -> QA RAG smoke | `GATEWAY_RAG_E2E_SMOKE=1 go test ./internal/integration -run '^TestGatewayRAGE2ESmoke$' -count=1 -v` | available（2026-07-01 新增；默认 skip；本轮只跑默认 skip 编译检查） | 需要 Gateway/Auth/Redis/Knowledge runtime/QA/AI Gateway 和可用 chat profile/provider；覆盖最小 answer/citation，不覆盖 MCP/前端/#125。 |
...
| 2026-07-03 | Codex docs watch | `develop@ce0b4774` | 复核 #527/#440/#529/#531：QA 的 Document MCP report tools env-gated smoke 可连接 Document Streamable HTTP `/mcp` endpoint；Gateway -> Knowledge -> QA RAG 依赖当前 Knowledge runtime，而不是旧独立 Parser 服务；Knowledge MCP server 已存在但默认 QA 接入仍未从内置 `search_knowledge` 收敛到四个 `knowledge__*` 目标工具。完整前端/#125/真实 provider E2E 仍未证明。 |
| 2026-07-03 | Codex local RAG/MCP fix | working tree | 复核本地 Gateway/QA/Knowledge/MCP：QA active config 已启用四个 `knowledge__*` Knowledge MCP 工具，默认 KB 列表为空；Knowledge MCP `search` 空 KB 检索可跨多个 runtime KB 返回结果；完整前端/#125/真实 provider Agent E2E 仍需后续证明。 |
