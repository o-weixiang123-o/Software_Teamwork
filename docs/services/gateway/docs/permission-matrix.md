...
| Knowledge 资源 | `/api/v1/knowledge-bases/**`、`/api/v1/documents/**`、`/api/v1/knowledge-queries` | `bearerAuth` | 注入用户上下文，统一 envelope；标准用户默认有 `knowledge:read`，可查看知识库和使用 `knowledge-queries` 检索；知识库和文档增删改入口由 Gateway 先按 `knowledge:write` / `knowledge:admin` / `system:admin` 预拦截。 | `knowledge` 校验知识库、文档、查询范围、可见性和写权限；document detail/chunks/content 需要 `knowledgeBaseId`。 |
