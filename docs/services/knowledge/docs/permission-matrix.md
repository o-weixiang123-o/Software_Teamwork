...
| 权限字符串 | `knowledge:read`、`knowledge:write`、`knowledge:admin`、`system:admin`、`admin:parser-config:write`。 |
...
| --- | --- | --- | --- | --- |
| 查看有权限知识库 | 允许，`standard` 默认具备 `knowledge:read`。 | 允许。 | 允许。 | 按知识库可见性、创建人或后续 ACL 过滤；Gateway 和 Knowledge 都不得把读权限误判为管理权限。 |
| 创建知识库 | 默认不允许，除非授予 `knowledge:write`。 | 允许。 | 允许。 | Gateway 先按 `knowledge:write` / `knowledge:admin` / `system:admin` 预拦截；Knowledge 仍复核。 |
| 更新知识库 | 默认不允许，除非授予 `knowledge:write` 且有资源权限。 | 允许。 | 允许。 | 软删除资源不可更新；Gateway 对写入口先做同一组写权限预拦截。 |
| 删除知识库 | 默认不允许，除非授予 `knowledge:write` 且有资源权限。 | 允许。 | 允许。 | 需要幂等软删除和后续清理；Gateway 对写入口先做同一组写权限预拦截。 |
| 上传/更新/删除文档 | 默认不允许，除非授予 `knowledge:write` 且可访问知识库。 | 允许。 | 允许。 | 需校验知识库状态和 RAGFlow runtime 处理边界；Gateway 对写入口先做同一组写权限预拦截。 |
...
| --- | --- | --- |
| Knowledge RAGFlow runtime | 文档存在、状态允许处理、调用方有处理权限；runtime token 和租户上下文由 Knowledge adapter 统一注入。 | 解析文档、生成 chunks、执行 embedding、写入索引并支持检索。 |
...
| RAGFlow runtime 返回认证失败或 token 错配 | `502 dependency_error`；这是服务间配置或下游依赖问题，不表示浏览器登录态过期。 |
