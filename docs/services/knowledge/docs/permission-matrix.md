...
| 资源事实 | Knowledge runtime 保存知识库文档、处理状态、chunks 和索引事实；Knowledge PostgreSQL 可选保存 parser configs 和迁移兼容字段。 |
| 文件事实 | 当前 Knowledge runtime 路径由 Knowledge runtime 保存原始 bytes；Knowledge 公开响应不得暴露 runtime object key、bucket、内部 URL 或历史 `file_ref`。 |
...
| 上传/更新/删除文档 | 默认不允许，除非授予 `knowledge:write` 且可访问知识库。 | 允许。 | 允许。 | 需校验知识库状态和 Knowledge runtime 处理边界。 |
...
| --- | --- | --- |
| Knowledge runtime | 文档存在、状态允许处理、调用方有处理权限。 | 解析文档、生成 chunks、执行 embedding、写入索引并支持检索。 |
...
| Knowledge runtime 返回认证失败或 token 错配 | `502 dependency_error`；这是服务间配置或下游依赖问题，不表示浏览器登录态过期。 |
