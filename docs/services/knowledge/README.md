...
| [数据模型](docs/data-models.md) | Knowledge Service 的 Knowledge runtime 映射、parser-config PostgreSQL 资源和历史兼容字段。 |
...
`services/knowledge-runtime` Knowledge runtime。Gateway -> Knowledge owner route smoke
和 Gateway -> Knowledge -> QA RAG env-gated smoke 已存在，但完整上传到检索、
rerank、MCP/Gateway 总入口仍由后续跨服务 smoke 覆盖。当前 adapter 不再直接调用
File Service、Redis/asynq 或旧 `services/parser`；runtime 内部的 MinIO、
...
- 当前 Go 进程是 Knowledge runtime contract adapter；`cmd/adapter` 代理 Gateway `/internal/v1/**` 契约到 `services/knowledge-runtime`。
- 原始文档解析、切块、embedding、索引和检索支持由 `services/knowledge-runtime` 的 Knowledge runtime API/worker 提供；Knowledge Go 进程不承载 PaddleOCR、PaddlePaddle、OpenCV、CUDA 或模型加载依赖。
...
| 文档解析协调与切片 | 通过 Knowledge runtime 创建 dataset/document、触发 parse，并读取 runtime 输出的文档状态和 chunks。 |
| 向量索引 | 通过 Knowledge runtime 完成 embedding 和索引写入；Knowledge adapter 负责公开契约映射和业务状态归一化。 |
| 删除清理 | 当前 adapter 对外提供文档软删除契约；runtime 内部对象/索引生命周期由 Knowledge runtime 处理，后续跨服务 smoke 仍需证明清理副作用。 |
...
`knowledge` 拥有；Knowledge runtime 是 Knowledge 的实现细节，不作为独立产品服务暴露给 gateway。
...
knowledge service
   ...
   +--> Knowledge runtime API/worker for documents, chunks, embedding, index, retrieval
...
- `documents/{documentId}/content`：知识库原始文件流，底层 bytes 由 Knowledge runtime/adapter 映射返回。
...
知识库文档上传入口由 `knowledge` 拥有，精确 method、path 和 multipart schema 以 Gateway OpenAPI 为准。Knowledge Service 负责接收 gateway 转发的 multipart、创建知识库文档资源、维护处理状态、chunks、embedding、索引和检索公开响应。当前 adapter 模式通过 Knowledge runtime API/worker 保存文档、解析 PDF、生成 chunks、执行 embedding、写入索引并提供检索能力；runtime 的对象存储、索引和 provider 细节不得泄露到 gateway 或前端响应。gateway 不能直接解析文件或操作索引后端。
...
| `dependency_error` | `502` | PostgreSQL、MinIO、Knowledge runtime、Elasticsearch/索引后端、AI Gateway 或其他依赖失败。 |
