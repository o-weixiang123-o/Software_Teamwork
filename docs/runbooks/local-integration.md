...
从空数据状态复现启动问题时，按项目范围停止服务并清理 compose 数据卷；本会话默认不删除镜像：
...
| Knowledge retrieval | `Knowledge retrieval stage: ...`、无 expected hit 或 rerank trace 异常 | 查 `.local/logs/gateway.log`、`.local/logs/knowledge.log` 和 `.local/logs/ai-gateway.log`；确认 `knowledge-queries` 返回 ready 文档 chunk，runtime 显式配置 `KNOWLEDGE_RUNTIME_RERANK_FACTORY=AI_GATEWAY` 和匹配的 rerank profile/model，并在 AI Gateway `provider_invocations` 里看到 `caller_service=knowledge`、`operation=reranking`。 |
