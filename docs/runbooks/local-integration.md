...
```text
Docker infra -> host Knowledge runtime/adapter -> host backend -> frontend
```
...
从空 Docker 状态复现启动问题时，按项目范围清理 compose 数据卷、Knowledge runtime
本地 Elasticsearch 资源和本项目镜像：
...
- `HF_ENDPOINT=https://hf-mirror.com` 用于 runtime worker 首次导入 deepdoc OCR/vision
  模块时下载 `InfiniFlow/deepdoc`。如果 `.local/logs/knowledge-runtime-worker.log`
  出现 `LocalEntryNotFoundError`、`ConnectTimeout` 或 `InfiniFlow/deepdoc` 下载失败，
  先确认 `HF_ENDPOINT`，或改为企业内部 HuggingFace 镜像。
...
- 如果某个服务日志显示 `bind: address already in use`，用
  ...
  监听默认端口；先停止旧进程，或在 `deploy/.env` 修改对应端口后重新启动。
...
- 默认 Knowledge adapter 不自动触发 runtime ingestion。需要证明真实上传、解析、
  embedding、索引和检索时，用 `./scripts/local/run-knowledge-parse-stack.sh`
  启动 runtime API、worker、Knowledge adapter 和本地 Elasticsearch。第一次启用前
  先执行 `cp deploy/.env.example deploy/.env`，并在本地 `deploy/.env` 中显式设置
  ...
  变量。脚本默认使用 `deploy/Dockerfile.elasticsearch-local` 启动
  `software-teamwork-knowledge-elasticsearch`；如使用外部 Elasticsearch，设置
...
API key 仍只应保存在本地 `deploy/.env`、受控 secret store 或浏览器提交请求中，不要提交。
