...
| Local integration runbook / README | current develop + PR Docker policy update | 默认联调路径是 Docker infra -> host backend -> frontend；根级 Compose 默认基础设施清单为 PostgreSQL、Redis、MinIO、minio-init 和 Elasticsearch，业务服务、migration、seed 和 Knowledge runtime 通过 `scripts/local/dev-up.sh` / `run-backend.sh` / runtime runbook 走宿主机路径；默认下载源保持官方，`--china` 显式为中国大陆网络切换 Docker/Go/uv/runtime artifact 镜像；AI Gateway placeholder profile 的 degraded/placeholder readiness 不等于真实 provider 可用。 |
...
| AI Gateway embeddings | 已实现 | `POST /internal/v1/embeddings`、profile model exact-match、response count/index 校验、usage aggregate。 | AI Gateway OpenAPI embedding endpoint。 | Knowledge runtime 已提供 `AI_GATEWAY` embedding provider；真实 provider indexing smoke 需以 `provider_invocations` 证明未绕过网关。 | #121、#225、#152 |
