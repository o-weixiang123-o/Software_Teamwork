...
| Docker policy | `python3 scripts/check_docker_policy.py`；验证根级 Compose 默认服务只包含 `postgres`、`redis`、`qdrant`、`minio`、`minio-init`；唯一允许的 profile/build 是可选 `knowledge-runtime` profile 下的 `elasticsearch`，基础设施镜像默认值不使用 `latest`，`deploy/.env.example` 不启用第三方镜像源或 profile。 |
