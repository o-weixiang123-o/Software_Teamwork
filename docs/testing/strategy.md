...
| Docker policy | `python3 scripts/check_docker_policy.py`；验证根级 Compose 默认服务只包含 `postgres`、`redis`、`qdrant`、`minio`、`minio-init`；唯一允许的 profile 服务是可选 `knowledge-runtime` profile 下的 `elasticsearch`，根 Compose 不包含 `build:`，基础设施镜像默认值不使用 `latest`，`deploy/.env.example` 不启用第三方镜像源或 profile。 |
...
| Compose | `docker compose -f deploy/docker-compose.yml --env-file deploy/.env.example config --quiet`；默认服务清单只能是五个 infra 服务。可选 `elasticsearch` 只允许在 `knowledge-runtime` profile 中出现，并由本地 `.env` 显式启用。 |
