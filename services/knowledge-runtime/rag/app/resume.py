...
def _call_llm(prompt: str, scope_id , lang: str) -> Optional[dict]:
    """
    ...
    try:
        ...
        llm =  LLMBundle(scope_id, LLMType.CHAT, lang=lang)
...
def _extract_basic_info(indexed_text: str, scope_id , lang: str) -> Optional[dict]:
    ...
    return _call_llm(prompt,scope_id, lang)


def _extract_work_experience(indexed_text: str, scope_id , lang: str) -> Optional[dict]:
...
def _extract_education(indexed_text: str, scope_id , lang: str) -> Optional[dict]:
...
def _extract_project_experience(indexed_text: str, scope_id , lang: str) -> Optional[dict]:
...
def parse_with_llm(indexed_text: str, lines: list[str], scope_id , lang: str) -> Optional[dict]:
    """
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            future_basic = executor.submit(_extract_basic_info, indexed_text, scope_id , lang)
            future_work = executor.submit(_extract_work_experience, indexed_text, scope_id , lang)
            future_edu = executor.submit(_extract_education, indexed_text, scope_id, lang)
            future_project = executor.submit(_extract_project_experience, indexed_text, scope_id , lang)
...
def parse_resume(filename: str, binary: bytes, scope_id , lang: str = "Chinese") -> tuple[dict, list[str], list[dict]]:
    """
    ...
    # Phase 2: Parallel LLM structured extraction
    resume = parse_with_llm(indexed_text, lines, scope_id , lang)
...
def chunk(filename, binary, scope_id, from_page=0, to_page=MAXIMUM_PAGE_NUMBER,
    ...
    Args:
        ...
        **kwargs: Other parameters (parser_config, kb_id, scope_id, etc.)
    Returns:
        Document chunk list
    """
    if callback is None:
    ...
    try:
        ...
        # Parse resume
        resume, lines, line_positions = parse_resume(filename, binary, scope_id , lang)
