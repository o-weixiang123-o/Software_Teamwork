...
- 如果希望 AI Gateway 的本地默认 profile 也自动指向同一组 provider，在
  ...
  `./scripts/local/dev-up.sh` 才会在 SQL demo seed 后执行
  `services/ai-gateway/cmd/local-seed`，使用
  `AI_GATEWAY_CREDENTIAL_ENCRYPTION_KEY` 加密 API key，并更新
  `default-chat`、`default-embedding`、`default-rerank`。未填写的模型会跳过。
  如果配置了 chat model，还会新增或激活 QA `llm_config_versions` 中匹配
  ...
  profile 不一致。QA 的 persisted LLM 配置由 `cmd/local-seed` 同步。
...
`AI_GATEWAY_LOCAL_*_MODEL`，随后才会执行 `go run ./cmd/local-seed` 加密写入本地
AI Gateway provider credential；未启用该开关时不会运行这个 Go seed。配置 chat model
时也会激活匹配的 QA `llm_config_versions` 版本。
