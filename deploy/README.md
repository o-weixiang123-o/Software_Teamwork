...
```text
Docker: postgres + redis + qdrant + minio + minio-init + Elasticsearch
...
source deploy/.env
set +a
docker rm -f "${KNOWLEDGE_RUNTIME_ELASTICSEARCH_CONTAINER:-software-teamwork-knowledge-elasticsearch}" 2>/dev/null || true
docker volume rm "${KNOWLEDGE_RUNTIME_ELASTICSEARCH_VOLUME:-software-teamwork-knowledge-elasticsearch-data}" 2>/dev/null || true
docker rmi \
...
HuggingFace 的地址，默认使用 `https://hf-mirror.com`。大陆网络下不要删除这项；如果
worker 日志出现 `huggingface_hub.errors.LocalEntryNotFoundError`、`ConnectTimeout`
或 `InfiniFlow/deepdoc` 下载失败，先确认 `deploy/.env` 中有 `HF_ENDPOINT`，然后重新运行
`./scripts/local/run-knowledge-parse-stack.sh`。
...
- 如果希望 AI Gateway 的本地默认 profile 也自动指向同一组 provider，在
  `deploy/.env` 中额外填写一组本地 seed 变量。`./scripts/local/dev-up.sh`
  会在 SQL demo seed 后执行 `services/ai-gateway/cmd/local-seed`，使用
  `AI_GATEWAY_CREDENTIAL_ENCRYPTION_KEY` 加密 API key，并更新
  `default-chat`、`default-embedding`、`default-rerank`。未填写的模型会跳过。
  ...
  如果 `MODEL_ID` 或 `DOCUMENT_AI_GATEWAY_MODEL` 仍是 `local-placeholder-chat`，
  `./scripts/local/run-backend.sh` 会在本次 host-run 启动里自动改用
  `AI_GATEWAY_LOCAL_CHAT_MODEL`，避免 QA/Document 请求模型名和 `default-chat`
  profile 不一致。

- `./scripts/local/run-knowledge-parse-stack.sh` 在 host 模式下会默认用
  `deploy/Dockerfile.elasticsearch-local` 构建并启动一个本地 Elasticsearch 容器，
  监听 `KNOWLEDGE_RUNTIME_ES_URL` 的端口，并生成
  `.local/knowledge-runtime/service_conf.yaml` 供 runtime API 和 worker 使用。
  如果你要使用已有宿主机或外部 Elasticsearch，设置
  `KNOWLEDGE_RUNTIME_START_ELASTICSEARCH=0`，并把 `KNOWLEDGE_RUNTIME_ES_URL`
  改成实际地址。
- runtime worker 第一次启动会按需下载 `InfiniFlow/deepdoc` 等 deepdoc 模型。大陆网络
  默认通过 `HF_ENDPOINT=https://hf-mirror.com`；下载期间 adapter `readyz` 会一直等待
  worker heartbeat。脚本默认最多等待 300 秒，可用 `KNOWLEDGE_ADAPTER_READY_TIMEOUT_SECONDS`
  调整。
...
`deploy/seeds/004-qa-default-knowledge-base.sql`。如果 `deploy/.env` 填了
`AI_GATEWAY_LOCAL_PROVIDER_BASE_URL`、`AI_GATEWAY_LOCAL_PROVIDER_API_KEY` 和至少一个
`AI_GATEWAY_LOCAL_*_MODEL`，随后还会执行 `go run ./cmd/local-seed` 加密写入本地
...
| Knowledge adapter `/readyz` says `task executor heartbeat is missing` | Runtime worker exited or is still downloading deepdoc models | Check `.local/logs/knowledge-runtime-worker.log`; for mainland China keep `HF_ENDPOINT=https://hf-mirror.com` in `deploy/.env` |
